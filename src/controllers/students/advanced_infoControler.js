...
export const createStudentAdvancedInfo = async (req, res) => {
  try {
    ...
    const searchStudent = await StudentProfile.findById(student_id);

    if (!searchStudent) {
      return res
        .status(404)
        .json({ message: "Student Advanced Info not found" });
    }

    const newAdvancedInfo = await StudentAdvancedInfo.create({
      aadhar_number,
      bank_name,
      account_number,
      ifsc_code,
    ...
    searchStudent.advanced_info_id = newAdvancedInfo._id;
...
export const updateStudentAdvancedInfo = async (req, res) => {
  try {
    ...
    if (!searchStudent) {
      return res
        .status(404)
        .json({ message: "Student Advanced Info: student not found" });
    }

    // 2. Ensure student has linked advanced info
    const advancedInfoId = searchStudent.advanced_info_id;
    if (!advancedInfoId) {
      return res
        .status(404)
        .json({ message: "AdvancedInfo not linked to this student." });
    }
    ...
    if (!updateAdvancedInfo) {
      return res
        .status(404)
        .json({ message: "AdvancedInfo record not found." });
    }
    return res.status(200).json({
      success: true,
