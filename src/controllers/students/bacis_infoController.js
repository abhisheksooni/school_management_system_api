...
export const createStudentBasicInfo = async (req, res) => {
  try {
    ...
    const newBasicInfo = await StudentBasicInfo.create({
      student_id,
      form_number,
      // student_dob,
      dob,
    ...
    searchStudent.basic_info_id = newBasicInfo._id;
...
export const updateStudentBasicInfo = async (req, res) => {
  try {
    ...
    const getStudent = await StudentBasicInfo.findById(student_id);

    if (!getStudent) {
      return res
        .status(404)
        .json({ message: "updateStudentBasicInfo: student not found" });
    }

    const updateBasicInfo = await StudentBasicInfo.findOneAndUpdate(
    ...
    if (!updateBasicInfo) {
      return res
        .status(404)
        .json({ message: "BasicInfo record not found for this student." });
    }
